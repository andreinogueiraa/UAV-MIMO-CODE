...
%
SE_MR_tot = zeros(K*L,nbrOfSetups);
SE_ZF_tot = zeros(K*L,nbrOfSetups);
SE_SMMSE_tot = zeros(K*L,nbrOfSetups);
SE_RZF_tot = zeros(K*L,nbrOfSetups);
SE_MMMSE_tot = zeros(K*L,nbrOfSetups);
...
for n = 1:nbrOfSetups
    ...
    for m = 1:length(Mrange)
        ...
        for s = 1:length(fRange)
            ...
            SE_MR_tot(:,n) = SE_MR(:);
            SE_ZF_tot(:,n) = SE_ZF(:);
            SE_SMMSE_tot(:,n) = SE_SMMSE(:);
            SE_RZF_tot(:,n) = SE_RZF(:);
            SE_MMMSE_tot(:,n) = SE_MMMSE(:);
...
plot(sort(SE_MMMSE_tot(:)),linspace(0,1,nbrOfSetups*K*L),'r-','LineWidth',1);
plot(sort(SE_SMMSE_tot(:)),linspace(0,1,nbrOfSetups*K*L),'b','LineWidth',1);
plot(sort(SE_RZF_tot(:)),linspace(0,1,nbrOfSetups*K*L),'k','LineWidth',1);
plot(sort(SE_ZF_tot(:)),linspace(0,1,nbrOfSetups*K*L),'g','LineWidth',1);
plot(sort(SE_MR_tot(:)),linspace(0,1,nbrOfSetups*K*L),'y','LineWidth',1);

xlabel('SE [bit/s/Hz/cell]');
ylabel('CDF');
legend('M-MMSE','S-MMSE','RZF','ZF','MR','Location','NorthWest');